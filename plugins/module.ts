...
class Loader {
    public contents: Array<string> = reactive([  ])
    constructor () {

    }

    addLoad (_content: string): { success: true, solve: () => boolean } {
        this.contents.push(_content)

        return {
            success: true,
            solve: () => {
                this.contents = this.contents.filter(__content => __content !== _content)
                return true
            }
        }
    }
...
export default defineNuxtPlugin((nuxtApp): {
    ...
    return {
        provide: {
            loadingView: reactive(new Loader()),
